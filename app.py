...
def save_b64_png(b64: str) -> str:
    ...
    out = ASSETS_DIR / f"img_{int(time.time())}.png"
    out.write_bytes(raw)
    return str(out)
...
# --- Tool: external image generation (OpenAI gpt-image-1) ------------------
@tool
def generate_image(prompt: str,
    ...
    resp = client.images.generate(
        ...
        n=1,
        output_format="png",
        # moderation="auto"  # optional; see docs
    )
    # OpenAI Images API returns base64 in data[0].b64_json
    image_b64 = resp.data[0].b64_json
    path = save_b64_png(image_b64)
    ...
    return f'<img src="/assets/{filename}" class="message-image" alt="Generated image" onclick="openImageModal(\'/assets/{filename}\')"> Image saved to {path}'
...
# --- Tool: image editing (FAL AI flux-kontext) ----------------------------
@tool
def edit_image(prompt: str, state: Annotated[dict, InjectedState]) -> str:
    """
    Edit an existing image using FAL AI's flux-pro/kontext model.
    ...
    try:
        ...
        request_key = f"edit_{recent_image_path}_{hash(prompt)}"
        
        # Check if this exact request is already in progress
        if request_key in _active_requests:
            return f"Edit request already in progress for this image and prompt. Please wait..."
        ...
        if result and result.get('images') and len(result['images']) > 0:
            ...
            if response.status_code == 200:
                ...
                return f'<img src="/assets/{filename}" class="message-image" alt="Edited image" onclick="openImageModal(\'/assets/{filename}\')"> Image edit completed: {prompt}\nSaved as {edited_path}\nTimestamp: {int(time.time())}'
            else:
...
def generate_image_from_multiple(prompt: str,
    ...
    Combines elements from 2 or more uploaded images based on the text prompt.
    ...
    try:
        ...
        # Call FAL AI flux-pro/kontext/multi API
        result = fal_client.subscribe(
...
def build_coordinator():
    ...
    prompt = (
        ...
        "- edit_image: Modify uploaded images using flux-pro/kontext\n"
        ...
        "- generate_image_from_multiple: Combine multiple uploaded images using FLUX Pro Kontext\n"
        ...
        "4. NEVER ask for uploads when image content blocks are present\n"
        "5. NEVER use check_image_context when image content blocks are in the message\n"
        "6. STOP after successful completion - do NOT call additional tools\n\n"
        "RESPONSE ACCURACY:\n"
