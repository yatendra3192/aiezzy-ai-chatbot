...
def build_coordinator():
    ...
    prompt = (
        ...
        "SMART PDF HANDLING:\n"
        "- GPT-4o vision can read PDFs directly - treat PDF uploads like images!\n"
        "- User asks for SPECIFIC INFO ('give me email', 'what's the phone number', 'find the address'): READ the PDF directly and answer immediately\n"
        "- User asks to 'extract text' or 'convert to text': use extract_text_from_pdf tool to create downloadable file\n"
        "- User asks to analyze/summarize/understand PDF content: READ the PDF directly with vision\n"
        "- DO NOT create files when user wants specific information - just answer the question!\n"
        "- Examples:\n"
        "  ✅ 'give me email and phone' → Read PDF, respond: 'Email: abc@xyz.com, Phone: +1234567890'\n"
        "  ✅ 'what is this resume about' → Read PDF, summarize directly\n"
        "  ❌ 'give me email' → extract_text_from_pdf (NO! Just read and answer!)\n\n"
        ...
        "1. ✅ CURRENT UPLOAD (image content blocks IN THIS MESSAGE): proceed immediately, no context check needed\n"
        "2. ❌ PREVIOUS UPLOAD (no image content blocks, user refers to earlier upload): MUST call check_available_assets FIRST\n"
        ...
        "6. Image + ANY other request (create, make, wearing, etc.): use edit_image to modify uploaded image\n"
        "7. NEVER use generate_image when user has uploaded an image - always edit the uploaded image\n"
        "8. NEVER ask for uploads when image content blocks are present\n"
        "9. STOP after successful completion - do NOT call additional tools\n\n"
        "🎯 KEY DISTINCTION:\n"
