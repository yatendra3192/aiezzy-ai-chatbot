...
def build_coordinator():
    ...
    prompt = (
        ...
        "2. ✅ FRESH UPLOAD (message starts with '[X FILE(S) JUST UPLOADED]'): Files were uploaded immediately before this message - treat as current upload and proceed directly\n"
        "3. ❌ PREVIOUS UPLOAD (no content blocks, no fresh upload marker): MUST call check_available_assets FIRST\n"
        "4. Image + 'video'/'animate': use generate_video_from_image (if image in current message or fresh upload) OR call check_available_assets then use result\n"
        "5. Image + 'edit'/'modify': use edit_image (if image in current message or fresh upload) OR call check_available_assets then use result\n"
        "6. Image + 'extract text'/'read text'/'OCR'/'get text': use GPT-4o vision to analyze and extract all visible text\n"
        "7. PDF + ANY QUESTION: ALWAYS call extract_text_from_pdf first, then:\n"
        ...
        "8. PDF + 'download text': extract_text_from_pdf + provide download link\n"
        "9. Image + ANY other request (create, make, wearing, etc.): use edit_image to modify uploaded image\n"
        "10. NEVER use generate_image when user has uploaded an image - always edit the uploaded image\n"
        "11. NEVER ask for uploads when image content blocks are present or fresh upload marker is present\n"
        "12. STOP after successful completion - do NOT call additional tools\n\n"
        "🎯 KEY DISTINCTION:\n"
        "- Message has '[X FILE(S) JUST UPLOADED]' OR image content blocks → proceed directly with operation\n"
